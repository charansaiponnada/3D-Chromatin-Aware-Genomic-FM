...
| 10 | **Permeability gate disabled for Phase D** (2026-08-31, **PI decision**, amends failure mode F2) | **`use_permeability=False`** (`--no-permeability`): the `w_gate` module is not constructed and `p` stays `None`, so τ = 1/(Δ·\|A\|) exactly as in the baseline | F2 fixed `b_g = -4` to trade init-equivalence against gradient scale, giving p = 0.018150. But τ = 1/(Δ·\|A\| + p) means **a floor on p is a ceiling on τ**, exactly as `dt_floor` is: τ ≤ 1/p ≈ **55 tokens**, *below* the uncorrected reference bound F4 was fixed to remove, and paid by the **structural arm only** — the baseline has no p term at all. Every τ reported before 2026-08-31 omitted p and so overstated the structural arm's horizon ~10× (reported median 487.3; true ≈ 51, i.e. ~8.5× *shorter* than the baseline's 434.7). The arms were therefore not comparable on the very quantity the mechanism acts through. D2 measured the gate as never engaging — mean p 0.017234/0.017193/0.017108 over 287,309,824 evaluations, full range [0.0133, 0.0197], all mass in one bin — so nothing measured is given up. Option D (add p to the baseline) is excluded by the standing rule against weakening the baseline. | Low — a per-run flag; `use_permeability=True` restores F2 exactly. Verified: `test_model.py` 21/21 (including "larger p shortens tau" and "baseline reports no p term"), `test_phase4_wiring.py` 60/60, and an end-to-end `--smoke` run under the full Phase D flag set confirming `p_bias: None` and `use_permeability: false` in `run_config.yaml`. Params 7,758,386 vs baseline 7,725,312 = +0.428%. |
